...
import { cloneDeep, isArray, isObject, isNaN, transform } from "./util";
...
Object.assign(Parameters.prototype, {
  ...
  _permitObject: function(params, filters) {
    for (var key in filters) {
      ...
      if (isArray(filtersArray) && (param = this._fetch(key))) {
        if (Parameters._isPrimitive(param)) {
          continue;
        }
        if (
          isArray(param._params) ||
  ...
  clone: function() {
    ...
    parameters._attrs = cloneDeep(this._attrs);
    parameters._filters = cloneDeep(this._filters);
    parameters._params = (function() {
      if (isArray(_this._params)) {
        return Parameters._cloneArray(_this._params);
      } else if (isObject(_this._params)) {
